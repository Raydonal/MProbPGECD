...
\bigskip 
\begin{regras}
Leia com atenção as perguntas. A prova deve ser claramente resolvida. Seja organizado. Pode fazer uso da calculadora e de materiais impressos, como livros, manuais ou quaisquer textos. É expressamente {\bf proibido} o uso do computador e celular durante a prova. 
\end{regras}
\vspace{2ex}
...
\end{cases}
$$
Obtenha a função de densidade condicional de $Y$ dado $X,$ i.e., $f_{Y|X}(y|x).$
\end{problema}
...
\end{cases} 
$$
Obtenha $E(X|Y)$ e $E(X).$  {\bf Dica:}  Note que as variáveis $X$ e $Y$ são discretas.
\end{problema}


\begin{problema}{}{pr-3}
Suponha que a distribuição conjunta das variáveis aleatórias $X$ e $Y$ está dada por
\begin{center}
	\begin{tabular}{|c|c|c|} \hline
		$X \diagdown Y$ & 1 & 2\\ \hline
		1 & 0,1 & 0,2 \\ \hline
		2 & 0,4 & 0,3 \\ \hline
	\end{tabular}
\end{center}
Calcule o coeficiente de correlação entre $X$ e $Y^2,$ i.e. ${\rm \rho}(X,Y^2).$ 
\end{problema}


\begin{problema}{}{pr-4}
 Seja $\{X n \}_{n\geq 1}$ uma sequência de variáveis aleatórias tais que $X_n \sim {\rm Exp }(n),$ i.e., para cada $n,$ a variável aleatória $X_n$ segue uma distribuição exponencial de parâmetro $n$. Demonstre que $X_n$ converge em probabilidade para 0.
\end{problema}
...
\begin{problema}{}{pr5}
Seja $\{X n \}_{n\geq 1}$ uma sequência de variáveis aleatórias tais que 
$$
P(X_n=0)= 1-\frac{1}{n^2} \quad \text{e} \quad P(X_n=n^2)=\frac{1}{n^2} .
$$
Demonstre que $X_n$ converge quase certamente (ache o limite $X$), mas não converge em $r$-ésima média para todo $r=1,2,\ldots$
\end{problema}
