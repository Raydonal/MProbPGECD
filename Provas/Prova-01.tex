...
\begin{enumerate} 
	...
	% e sendo a função de probabilidade $f(x)$  definida por
	% $$
	% f(x)=P(X=x) = \begin{cases}
		%           \frac{1}{2n+1} & \text{se} \quad x \in {\cal M}, \\
		%           0 & \text{caso contrário},
		%          \end{cases}
	% $$ em que ${\cal M}=\{-n,-(n-1), \ldots, -2,-1, 0, 1, 2, \ldots, (n-1), n \}.$
	% % e $A=\{1,2,3, \ldots , n-1, n\}.$
	
%	\item Compare $f(x)$ e $f(x|A)$ em termos de suas funções de distribuição de probabilidade acumuladas, i.e. entre $F(x)$ e $F(x|A)$  \\	
{\bf Dica:} 
...
%	\item[(c)] Encontre as densidades marginais de $X$ e de $Y$, isto é, $f_X(x)$ e $f_Y(y)$.
...
%	\item[(g)] Verifique se $R$ e $\Theta$ são independentes.
	\end{enumerate}
\end{problema}
